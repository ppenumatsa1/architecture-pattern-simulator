...
- When introducing new event payload schema, bump `metadata.schema_version` and add upcasters in `event_store/upcasters.py`.
- Preserve aggregate event ordering/versioning (`event_version` monotonic per aggregate).
